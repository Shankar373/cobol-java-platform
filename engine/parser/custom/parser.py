"""
engine/parser/custom/parser.py

Custom COBOL parser. Hand-written recursive-descent parser for fixed and
free-format COBOL.

Constructs supported (evidence level: DIFFERENTIALLY_VERIFIED or UNIT_TESTED):
  IDENTIFICATION / DATA / PROCEDURE DIVISION
  DATA ITEMS: PIC/PICTURE, USAGE COMP/COMP-3/COMP-5/DISPLAY, REDEFINES,
              OCCURS, OCCURS DEPENDING ON, VALUE, LEVEL 88
  STATEMENTS: MOVE, ADD, SUBTRACT, MULTIPLY, DIVIDE, COMPUTE,
              IF/ELSE/END-IF, EVALUATE/WHEN/END-EVALUATE,
              PERFORM (inline, thru, until, varying, times),
              DISPLAY, READ, WRITE, REWRITE, OPEN, CLOSE, DELETE, START,
              STRING, UNSTRING, INSPECT, SORT, GOBACK, EXIT, CONTINUE,
              STOP RUN, CALL, SET, INITIALIZE, GO TO, NEXT SENTENCE
  SQL:  EXEC SQL ... END-EXEC (captured as SQL_BLOCK nodes)
  CICS: EXEC CICS ... END-EXEC (captured as CICS_BLOCK nodes)
  COPY: resolved inline by the lexer preprocessor
  JCL:  Not parsed here; handled by transformation/jcl/

Refactored from cobol-java-modernization/modernize/parser.py.
Import paths updated to new package layout.
All parse behaviour preserved.
"""
import re
from engine.lexer.lexer import CobolToken
from engine.semantic.semantic_ir import SemanticIR, SemanticIRNode, SourceLocation
